...
class VSSMUNet(nn.Module):
    def __init__(self,
        ...
        self.DualDomainFusion = DualDomainFusion(in_channel=2)
    ...
    def apply_model(self, input, sens, post, iz_rec2=None):
        if sens != None:
            input = self.sens_reduce(input, sens, dim=1)
        else:
            input = ifft2c(input).permute(0, 3, 1, 2)
        if iz_rec2 != None:
            input = self.DualDomainFusion(input, iz_rec2.permute(0, 3, 1, 2))
        ...
        if sens != None:
            ...
            out_put = fft2c(out_put.permute(0,2,3,1))
        return out_put
    ...
    def forward(self, uk, mask, uk0, sens, post, iz_rec2=None):
        ...
        if self.out_put:
            ...
            if sens == None:
                i_rec = fastmri.complex_abs(i_rec2)
                plt.imshow(i_rec[0,:,:].abs().cpu().detach().numpy(), cmap=plt.cm.gray);plt.savefig(r'./rec.jpg');plt.close()
                return [i_rec, i_rec2]
            else:
                i_rec = fastmri.rss_complex(i_rec2, dim=1)
                return [i_rec]
        else:
            return k_out
        ...
        if self.posterior_sampler != None:
            with torch.no_grad():
                with self.posterior_sampler.model.ema_scope():
                    ...
                    cond = self.posterior_sampler.model.get_learned_conditioning(self.normalize(ui.permute(0,3,1,2))))
                    iz_rec2 = self.unnormalize(self.posterior_sampler.model.decode_first_stage(posterior)).permute(0,2,3,1)
        for layer in self.layers:
            x = layer(x, mask, uk0, sens, posterior, iz_rec2)
        return x
