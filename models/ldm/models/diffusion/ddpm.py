...
class LatentDiffusion_dc(LatentDiffusion):
    def __init__(self,
        first_stage_config,
        cond_stage_config,
        *args, **kwargs):
        super().__init__(first_stage_config=first_stage_config, cond_stage_config=cond_stage_config, *args, **kwargs)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   quantize_denoised=True, inpaint=True, plot_denoise_rows=False, plot_progressive_rows=True,
                   plot_diffusion_rows=True, **kwargs):
        'get a xrec after the data_consistency operation'
        use_ddim = ddim_steps is not None
        ...
        z, c, x, xrec, xc = self.get_input(batch, self.first_stage_key,
                                           ...
                                           return_original_cond=True,
                                           bs=N)
        mask = batch['mask'][:x.shape[0]]
        # xrec = self.data_consistency(xrec, x, mask)
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                ...
                # samples, z_denoise_row = self.sample(cond=c, batch_size=N, return_intermediates=True)
            x_samples = self.decode_first_stage(samples)
            x_samples = self.data_consistency(x_samples, x, mask)
            log["samples"] = x_samples
            # if plot_denoise_rows:
            #     denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
            #     log["denoise_row"] = denoise_grid
